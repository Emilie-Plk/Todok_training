...
import com.example.todok.domain.todo.UpdateTodoUseCase
...
class TodosViewModel @Inject constructor(
    ...
    private fun mapItem(categoryEntity: CategoryEntity, todoEntity: TodoEntity) = TodosViewStateItem.Todo(
        ...
        onLongClickEvent = EquatableCallback {
            viewModelScope.launch(coroutineDispatcherProvider.io) {
                updateTodoUseCase.invoke(todoEntity.copy(isDone = !todoEntity.isDone))
                delay(5.seconds)
                deleteTodoUseCase.invoke(todoEntity.id)
            }
        }
